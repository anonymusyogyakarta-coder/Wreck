...
// Path Termux - fix supaya Node.js bisa akses yt-dlp & ffmpeg
...
function checkTool(tool) {
  // Cek di Termux bin langsung
  const termuxPath = `${TERMUX_BIN}/${tool}`;
  try {
  ...
  try {
    const r = execSync(`which ${tool}`, { env: TERMUX_ENV, encoding: 'utf8' }).trim();
    if (r) return r;
  } catch {}
  return null;
}
...
async function ytdlpDownload(url, opts = {}) {
  ...
  const ytdlpBin = checkTool('yt-dlp') || 'yt-dlp';
  let cmd = `${ytdlpBin}`;
  if (opts.audio) {
...
async function handleTikTok(jid, url) {
  ...
  try {
    ...
    const ytBin = checkTool('yt-dlp') || 'yt-dlp';
    const cmd = `${ytBin} --no-watermark -f "best[ext=mp4]/best" --no-playlist -o "${outFile}" "${url}"`;
...
async function handleVideoToSound(jid, msg) {
  try {
    ...
    if (checkTool('ffmpeg')) {
      const ffmpegBin = checkTool('ffmpeg') || 'ffmpeg';
      await runCmd(`${ffmpegBin} -i "${vidPath}" -vn -acodec copy "${audPath}" -y`, 60000);
...
async function handleInstagram(jid, url) {
  ...
  try {
    ...
    const ytBin2 = checkTool('yt-dlp') || 'yt-dlp';
    const cmd = `${ytBin2} --no-playlist -o "${outFile}" "${url}"`;
    await runCmd(cmd, 120000);
...
async function startBot() {
  ...
  // Cek yt-dlp
  const ytdlpFound = checkTool('yt-dlp');
  const ffmpegFound = checkTool('ffmpeg');
  if (!ytdlpFound) {
    console.log(chalk.yellow('  ⚠️  yt-dlp tidak ditemukan! Install: pkg install yt-dlp -y'));
  } else {
    console.log(chalk.green(`  ✅ yt-dlp: ${ytdlpFound}`));
  }
  if (!ffmpegFound) {
    console.log(chalk.yellow('  ⚠️  ffmpeg tidak ada (fitur .vs terbatas)'));
  } else {
    console.log(chalk.green(`  ✅ ffmpeg: ${ffmpegFound}`));
  }
  console.log('');
